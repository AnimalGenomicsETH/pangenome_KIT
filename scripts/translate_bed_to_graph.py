...
def translate_bed_to_graph(gfa,bed):
    ...
    with open(bed,'r') as fin:
        for line in fin:
            s_coord, e_coord = (int(i) for i in line.rstrip().split()[1:])
            ...
            if s_node == e_node:
                print(f'{s_node}\t{s_offset}\t{e_offset}')
            else:
                for i in range(s_node,e_node+1):
                    if i == s_node:
                        print(f'{s_node}\t{s_offset}\t{boundaries[s_node]-boundaries[s_node-1]}')
                    elif i != e_node:
                        print(f'{i}\t{0}\t{boundaries[i]-boundaries[i-1]}')
                    else:
                        print(f'{e_node}\t{0}\t{e_offset}')
